...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
    ...
    # No fallback - must use AI-generated phrases
    raise RuntimeError(f"AI failed to generate phrases after {max_attempts} attempts")
...
def create_video_from_images_audio(image_files: list, audio_files: list, combined_audio: str, output_file: str):
    ...
    for i, (img_path, audio_info) in enumerate(zip(image_files, audio_files)):
        ...
        print(f"  Image {i+1}/{len(image_files)}: {duration:.2f}s (EN: {audio_info.get('en_duration', 0):.1f}s + FR: {audio_info.get('de_duration', 0):.1f}s)")
